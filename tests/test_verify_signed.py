...
class TestVerifySigned(unittest.TestCase):
    def test_wacz_valid_and_verify_sig(self):
        self.assertEqual(
            main(["validate", "--verify-auth", "-f", os.path.join(TEST_DIR, "valid_signed_example_1.wacz")]), 0
        )

    def test_wacz_valid_and_not_valid_sig(self):
        self.assertEqual(
            main(["validate", "--verify-auth", "-f", os.path.join(TEST_DIR, "invalid_signed_example_1.wacz")]), 1
        )
