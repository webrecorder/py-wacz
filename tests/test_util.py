...
from wacz.util import hash_content, validateJSON
...
class TestUtilFunctions(unittest.TestCase):
    def test_util_hash(self):
        ...
        self.assertEqual(hash_content("sha256", "test".encode("utf-8")), test_hash)

        test_hash = "md5:%s" % hashlib.md5("test".encode("utf-8")).hexdigest()
        self.assertEqual(hash_content("md5", "test".encode("utf-8")), test_hash)
